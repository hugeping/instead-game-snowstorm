...
obj {
	-"проход|отверстие|вход";
	nam = 'ворота';
	description = [[Разлом представляет из себя правильное овальное отверстие высотой около трёх метров.]];
}:attr 'scenery':disable()

obj {
	-"статуя|фигура|голем/ед,мр,од|ледяной человек|человек";
	nam = 'голем';
	init_dsc = function(s)
		p [[У стены стоит ледяная статуя.]]
	end;
	description = function(s)
		p [[Высота статуи около двух метров. У неё есть руки и ноги, но вместо головы лишь небольшой выступ.]]
	end;
	['before_Take,Climb,Push,Pull'] = [[Как ты это сделаешь?]];
...
страшный ледяной человек остановился и до тебя донёсся его громовой голос:^]]
				enable 'ворота'
				p [[-- Госпожа ждёт тебя!^]]
				p [[После этих слов голем размахнулся и ударил мощным кулаком в стену.]]
				p [[Стена с треском поддалась и в скале образовался проход в виде огромного отверстия.]]
				s:attr'animate'
			else
				p [[-- Госпожа ждёт тебя! -- прогремел голос.]]
			end
			return
		end
		return false
...
room {
	...
	dsc = function(s)
		...
		if not disabled 'ворота' then
			p [[В стене зияет огромное отверстие.]]
		end
	end;
}: with {
	'голем';
