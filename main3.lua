...
как множество чёрных тварей бросаются к тебе с потолка. Инстинктивно ты успеваешь выключить фонарик. Удивительно, но это успокаивает тварей. Некоторое время ты слышишь шум их крыльев. Затем всё затихает.]]
			s:attr'~light'
			return
		end
...
от ледяных стен, пола, потолка и массивных колонн. У южной стены расположена широкая лестница, ведущая наверх. Выход из зала находится на востоке.]];
		if not disabled'дверь' then
...
В нерешительности ты останавливаешься в нескольких шагах от неё.]]
	...
	obj {
		...
		description = [[Широкая парадная лестница с массивными перилами начинается от южной стены и поднимается вверх.]];
	}:attr 'scenery';
}
...
room {
	...
	o_to = '#двери';
	enter = function(s)
		p [[Ты поднялась по широкой лестнице и оказалась у входа в другой зал. Высокие двустворчатые
двери оказались незапертыми и ты вошла вовнутрь.]];
	end;
	dsc = [[Ты находишься в длинном, но узком зале с высоким потолком. Зал продолжается на запад и на восток. Выход в тронный зал ведёт вниз.]];
}: with {
	door {
		-"двери";
		nam = '#двери';
...
room {
	nam = 'Зал восток';
	title = 'Зеркальный зал (восток)';
}
room {
	nam = 'Зал запад';
	title = 'Зеркальный зал (запад)';
}
