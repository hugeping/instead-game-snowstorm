...
room {
	nam = 'обрыв';
	dsc = [[На севере гладкий пол пещеры заканчивается обрывом. Выход находится на юго-западе.]];
	out_to = 'пещера';
	['sw_to,w_to'] = 'пещера';
	['before_Drop,ThrowAt,Insert'] = function(s, w, ww)
		if not w ^ 'осколки' or (ww and not ww ^ '#обрыв') then
			return false
		end
...
свечению ты можешь примерно оценить размер пропасти.]];
	...
	obj {
		...
		before_Exam = function(s)
			p [[Глубокий разлом во льду. Света фонарика недостаточно, чтобы оценить его размеры.]];
			if s.seen then
				p [[^Но светящиеся осколки кристаллов на другой стороне указывают на то, что пропасть не очень широкая.]]
			end
		end;
		before_JumpOver = function(s)
			if not s.seen then
				p [[Расставаться с жизнью не входит в твои планы.]]
			else
				p [[TODO]]
			end
		end;
		...
		before_Enter = [[Тебе нужно найти маму, а не сбегать от проблем...]];
	}:attr 'scenery,container,open';
	Prop {
		nam = '#осколки';
		-"осколки";
		before_Exam = function(s)
			p [[Светящиеся осколки кристаллов на другой стороне указывают на то, что пропасть не очень широкая.]]
		end;
	}:disable();
}
