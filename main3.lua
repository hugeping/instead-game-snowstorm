...
{$fmt y,100%}{$fmt em|{$fmt c|Если Вы застрянете в игре, наберите "думать"}}]]
	};
	next_to = 'поле';
	exit = function(s)
...
room {
	nam = 'обрыв';
	dsc = [[На западе гладкий пол пещеры заканчивается обрывом. Выход находится на юго-западе.]];
	out_to = 'пещера2';
	w_to = "ообрыв";
	['sw_to,e_to'] = 'пещера2';
}:attr '~light': with {
...
obj {
	...
	before_Exam = [[Она закрывает ладонями лицо. Ты видишь на её ладонях капли крови.]];
